...
@Service
class UpdateOptionalUseCase(private val logger: Logger,
                            private val saveOptionalPort: SaveOptionalPort,
                            private val getMenuByIdPort: GetMenuByIdPort) {

    fun execute(optional: Optional, id: String): Mono<Optional> {
      optional.id = id
      logger.info("Update optional use case initiated. group=${optional.toJson()}")
      return getMenuByIdPort.getById(optional.menuId)
        ...
        .doOnNext { logger.info("Update optional use case done.") }
    }
}